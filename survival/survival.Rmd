...
cfit3 <- update(cfit2, .~. - batch:treatment)
...
# significance of batch and treatment is interaction term is omitted
cfit4 <- update(cfit3, .~. - batch)
anova(cfit3, cfit4)

cfit5 <- update(cfit3, .~. - treatment)
anova(cfit3, cfit5)
```
...
# for prettier graphs
ggsurvplot(fit1, data = workers, fun='event', conf.int = TRUE) 

# risk.table = TRUE,  risk.table.height = 0.25

# log rank test between KM survival curves.
surv <- Surv(time = workers$days, event = workers$death)
survdiff(surv ~ workers$treatment)
