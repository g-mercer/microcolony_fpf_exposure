...
```{r}
# intercept only
M3a.lme.d <- update(M3a.lme.d, method = "REML")

# extract residuals
E <- resid(M3a.lme.d, type = "normalized")
...
When you do this there looks to be more patterns in the residuals. Including batch makes residuals cluster more tightly around 0. Stick with M2a.lme. Including batch tackles residual patterns. 
...
p <- ggplot(sucrose, aes(x=treatment, y=corrected_consumption, color = treatment)) + 
  ...
  labs(y = "Total Sucrose Processed/Bee (mg)", colour = "Treatment") +
  ...
  annotate(geom = "text", x = 2.35, y = 4100, label = "bolditalic('p')~bold('= 0.0585')", fontface = 2, size = 6, parse = TRUE)

p
```

```{r}
cairo_pdf(file = "./output/sucrose_consumption_batch.pdf",   # The directory you want to save the file in
...
p1
 ...
 cairo_pdf(file = "./output/sucrose_consumption_treatment.pdf",   # The directory you want to save the file in
  width = 8, # The width of the plot in inches
  height = 6) # The height of the plot in inches
