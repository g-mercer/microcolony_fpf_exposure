...
M1.lmeE <- lme(drone_mass_mg ~ 1 + treatment,
               data = drone_mass_df,
               random = ~ 1 | colony_number/microcolony_number, method = "ML")
...
```{r}
boxplot(predict(M1.lmeE, level = 0, interval = confidence) ~ treatment, data = drone_mass_df)
```
...
M1.lmer <- lmer(drone_mass_mg ~ 1 + treatment + (1 | colony_number/microcolony_number), 
                data = drone_mass_df, REML = TRUE)
...
drone_mass_df$treatment <- relevel(drone_mass_df$treatment, "fpf-pure")
...
p1 <- ggplot(data = M1.lmer.df, aes(x = treatment, y = prediction, colour = as.factor(treatment))) + 
  ...
  labs(y = "Drone Mass (mg)", colour = "Treatment") +
