...
# report this p value. When treatment only model is compared to intercept
anova(M1.lmeE, M1.lmeD)
...
p1 <- ggplot(data = M1.lmer.df, aes(x = treatment, y = prediction, colour = as.factor(treatment))) + 
  ...
  annotate(geom = "text", x = 2.4, y = 50, label = "bolditalic('p')~bold('= 0.723')", fontface = 2, size = 6, parse = TRUE) +
