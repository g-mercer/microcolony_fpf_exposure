...
```{r}
drones$treatment <- relevel(drones$treatment, "control")
M4 <- glmmTMB(number_of_drones ~ 1 + treatment,
              data = drones,
              ziformula=~1,
              family = "nbinom2",
              REML = TRUE)
...
# fpf ci
drones$treatment <- relevel(drones$treatment, "fpf-pure")
...
p1 <- ggplot(data = predictions, aes(x = treatment, y = Prediction, colour = as.factor(treatment))) + 
  ...
  labs(y = "Number of Drones", colour = "Treatment") +
    
  # theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    
  ggtitle("Drone Number") +
