...
```{r}
pollen_day_1_plot <- plot_function(pollen_day_1, "#1AFF1A", theme(axis.title.x = element_blank()))

nectar_day_1_plot <- plot_function(nectar_day_1, "#4B0092", theme())
```
...
plot_function_1 <- function(x, y, z) {
  ...
  plot <- ggplot(data = x, aes(x = DALA_h, y = residue_measurement_ugkg)) +
          ...
          geom_hline(aes(yintercept = mean(pollen$residue_measurement_ugkg [pollen$DALA_h > 1])), color= y) +
...
pollen_day_3_plot_median <- plot_function_1(pollen_day_3, "#1A85FF", theme(axis.title.x = element_blank()))

nectar_day_2_plot_median <- plot_function_1(nectar_day_2, "#4B0092", theme() )

pollen_day_3_plot_median

cairo_pdf(file = "./pollen_day_3.pdf",   # The directory you want to save the file in
...
combined_plots <- plot_grid(
 ...
 pdf(file = "./combined_plots.pdf",   # The directory you want to save the file in
